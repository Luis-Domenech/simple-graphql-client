...
export const gen_clients_functions = async (data: GeneratorData, config: GeneratorConfig): Promise<string[]> => {
  ...
  functions.push([
    ...
    `${i(2)}else {`,
    `${i(3)}if (typeof obj === "object") return Object.keys(obj).length > 0 ? await is_in_selection_set(field_name, (obj as any)[parent_queue.shift()!], parent_queue) : null`,
  ...
  functions.push([
    ...
    `${i(5)}are_valid.push(await is_valid_operation_selection_set(operation, selection_set[selection], [...parent_queue, selection]))`,
    `${i(4)}}`,
  ...
  functions.push([
    ...
    `${i(1)}const use_conventions = opts.use_conventions ? opts.use_conventions : USE_CONVENTIONS`,
    `${i(1)}const prod = opts.production ? opts.production : process.env ? process.env.NODE_ENV ? process.env.NODE_ENV === 'production' : false : false`,
    ...
    `${i(3)}if (json.errors) {`,
    `${i(4)}if (log) console.log(\`Fetch failed on operation: \${operation_name}\`)`,
    `${i(4)}return { fetch_errors: json.errors.map(e => e.message) } as any`,
    `${i(3)}}`,
    ...
    `${i(4)}const find = Object.keys(selection_set).find((key) => key === 'errors')`,
    `${i(4)}if (!find && operation_response.errors) {`,
    `${i(5)}if (log) console.log(\`Fetch failed on operation: \${operation_name}\`)`,
    `${i(5)}return { fetch_errors: operation_response.errors.map(e => e.message) } as any`,
    `${i(4)}}`,
    `${i(3)}}`,
  ...
  functions.push([
    ...
    `${i(1)}if (log) console.log(\`Executing operation: \${operation_name}\`)`,
    `${i(1)}try {`,
    `${i(2)}const selection_set_is_valid = await is_valid_operation_selection_set(operation, sel_set, [])`,
    ...
    `${i(2)}const parsed_selection_set = await parse_selection_set(sel_set, operation_data.output_selection_sets)`,
    `${i(2)}// This is due to convention that all our inputs are named data`,
    `${i(2)}if (input) {`,
    `${i(3)}if (Object.keys(input).length > 0) {`,
    `${i(4)}await parse_operation_input_fields(input, operation_data.input_selection_sets)`,
    ...
    `${i(5)}let input_declarations = []`,
    `${i(5)}let input_values = []`,
    `${i(5)}Object.keys(input).map(key => {`,
    `${i(6)}const input_name = key`,
    `${i(6)}let argument_type = ''`,
    `${i(6)}try {`,
    `${i(7)}argument_type = GRAPHQL_OPERATION_DATA[operation]['input_types'][input_name]['complete_type'] ? GRAPHQL_OPERATION_DATA[operation]['input_types'][input_name]['complete_type'] : null`,
    `${i(6)}}`,
    `${i(6)}catch(e) {}`,
