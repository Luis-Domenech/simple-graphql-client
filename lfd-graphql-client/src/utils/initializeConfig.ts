...
import { CONFIG_NAMES_TO_IGNORE, DEFAULT_CONFIG, GENERATORS, logger, RECURSION_WARNING_THREHSOLD } from "../constants.js"
import { GeneratorConfigs, LFDGeneratorConfig } from "../lfd-types.js"
...
const initialize_from_user_config = (user_config: any, config: GeneratorConfig) => {
  ...
  config.global = {
    ...
    types_dir: wasProvided(user_config.types_dir) ? user_config.types_dir : DEFAULT_CONFIG.global.types_dir,
  ...
  const scalars: Map<string, ScalarOverrideData> = new Map()
  ...
  // Now that scalars have been added, now we add our scalars
  for (const [override, override_data] of Array.from(DEFAULT_CONFIG.types.scalars!.entries())) if (!scalars.get(override)) scalars.set(override, override_data)
  ...
  if (user_config.generator.client) {
    if (Object.keys(user_config.generator.client).length > 0) {
      ...
      else recursionOverrides = DEFAULT_CONFIG.client.recursion_overrides!
    }
  }
  ...
  config.client = {
    ...
    add_ignore: wasProvided(user_config.generator.client.add_ignore) ? user_config.generator.client.gen_hooks : DEFAULT_CONFIG.client.add_ignore,
  }
}
...
export const initialize_js_config = async (conf: Record<string, any>, config: GeneratorConfig) => {
  const js_config = convert_config_to_snake_case(conf) as LFDGeneratorConfig

  const user_config: any = {}

  Object.keys(js_config).map(key => {
    if (key !== "generator") {
      user_config[key] = js_config[key as keyof LFDGeneratorConfig]
    }
  })
...
export const initialize_ts_config = async (conf: Record<string, any>, config: GeneratorConfig) => {
  const ts_config = convert_config_to_snake_case(conf) as LFDGeneratorConfig

  const user_config: any = {}

  Object.keys(ts_config).map(key => {
    if (key !== "generator") {
      user_config[key] = ts_config[key as keyof LFDGeneratorConfig]
    }
  })
