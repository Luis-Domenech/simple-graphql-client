import { DEFAULT_CONFIG, DEFAULT_CONFIG_PATHS, DEFAULT_ENV_NAMES, DEFAULT_ENV_SUBS, logger, RECURSION_WARNING_THREHSOLD, REGEX, SUPPORTED_CONFIG_EXTENSIONS } from "../constants.js"
import { GeneratorConfig, SupportedConfigExtension } from "../types.js"
import fs from 'fs'
import path from 'path'
import { loadAll } from 'js-yaml'
import { initialize_json_config, initialize_js_config, initialize_yml_config } from "./initializeConfig.js"
...
const js_handler = async (config_file_path: string, config: GeneratorConfig) => {
  try {
    ...
    if (typeof conf === 'object') await initialize_js_config(conf, config)
    else if (typeof conf === 'function') await initialize_js_config(await conf(), config)
    else logger.error('Config file does not export a valid config object or function that returns a valid config object')
  }
  catch(e) {
    logger.error(`Error reading config file (${config_file_path})`)
  }
