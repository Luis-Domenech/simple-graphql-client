...
export const gen_clients_functions = async (data: GeneratorData, config: GeneratorConfig): Promise<string[]> => {
  ...
  functions.push([
    ...
    `${i(2)}const raw_response = await fetch(prod ? endpoint : dev_endpoint, {...fetch_options, method: 'POST', headers: headers, body: JSON.stringify({...operation_request, operationname: \`\${capitalize_first_word(operation) + capitalize_first_word(operation_type)}\`})})`,
    `${i(2)}try {`,
    `${i(3)}const json = await raw_response.json() as any`,
    `${i(3)}if (!json) {`,
    `${i(4)}if (log) console.log(\`Fetch failed on operation: \${operation_name}\`)`,
