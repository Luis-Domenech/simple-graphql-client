...
import { CLIENT_GENERATOR_DIRS, GRAPHQL_ROOT_OPERATIONS, logger, OPERATION_DATA_AUTO_GENERATED_COMMENT, REGEX, TO_REMOVE_STRING } from "../constants.js"
import { ClientGeneratorExport, FileData, GeneratorConfig, GeneratorData, ImportData } from "../types.js"
import { add_import, add_imports, add_relative_import, gen_fields_with_args_as_type, gen_fields_with_response_as_type, gen_imports, get_type_name, Indent, gen_data_fields, write_format_file, gen_primitive_object_types } from "../utils/index.js"
...
export const gen_client_types = async (data: GeneratorData, config: GeneratorConfig): Promise<string[]> => {
  ...
  client_types = [
    ...
    `type RecursionOverride<Output, N extends PositiveNumber<number>> = "__typename" extends keyof Output ? Output["__typename"] extends keyof TypeRecursionOverride ? TypeRecursionOverride[Output["__typename"]] : N : N`,
    ...
    [
      ...
      `${i(1)}signal?: AbortSignal | null | undefined`,
      `}`,
    ].join("\n"),
    ...
    [
      `type OutputFromUnion<SelectionSet, U> = (`,
      `${i(1)}SelectionSet extends object ?`,
      `${i(1)}IsUnion<U> extends true ? `,
      `${i(1)}(`,
      `${i(2)}(`,
      `${i(3)}"union_select' extends keyof NestedType<SelectionSet> ? // Check if we were given a union_select. We get NestedType here to get all object from the type`,
      `${i(3)}(`,
      `${i(4)}NestedType<SelectionSet>['union_select'] extends string ?`,
      `${i(4)}(`,
      `${i(5)}SelectUnionType<U, NestedType<SelectionSet>['union_select']> extends never ?`,
      `${i(5)}U :`,
      `${i(5)}SelectUnionType<U, NestedType<SelectionSet>['union_select']>`,
      `${i(4)}) :`,
      `${i(4)}U // If we weren't given a union, then return the union itself `,
      `${i(3)}) :`,
      `${i(3)}U`,
      `${i(2)})`,
      `${i(1)}) : `,
    ...
    `type BoundedOperationOutput<Output, N extends PositiveNumber<number>> = BoundedOperationOutput_<N, [], Output, true>`,
    [
      `type BoundedOperationOutput_<N extends PositiveNumber<number>, Depth extends unknown[] = unknown[], Output = Record<string, unknown>, FirstRecursion extends boolean = false> = (`,
    ...
    `export type BoundedOperationSelectionSetType<Output, N extends PositiveNumber<number>> = BoundedOperationSelectionSetType_<N, [], Output, true>`,
    [
      `type BoundedOperationSelectionSetType_<N extends PositiveNumber<number>, Depth extends unknown[] = unknown[], Output = Record<string, unknown>, FirstRecursion extends boolean = false> = (`,
      `${i(1)}N extends Depth['length'] ?`,
      `${i(1)}(`,
      `${i(2)}Output extends object ?`,
      `${i(2)}{ [K in keyof Output as Output[K] extends object ? never : K]?: boolean } : // Base case`,
      `${i(2)}boolean`,
      `${i(1)}) :`,
      `${i(1)}FirstRecursion extends true ?`,
      `${i(1)}(`,
    ...
    [
      ...
      `${i(1)}Record<string, unknown> :`,
      `${i(1)}null `,
      `)\n`,
    ].join("\n"),
    [
      `export type BoundedOperationResponse<SelectionSet, Output extends any | void, N extends PositiveNumber<number>> = { `,
      `${i(1)}output: (`,
      ...
      `${i(2)}(`,
      `${i(3)}SelectionSet extends true ? `,
      `${i(3)}BoundedOperationOutput_<N, [], NestedType<Output>, true> :`,
    ...
    [
      `type BoundedOperationResponse_<N extends PositiveNumber<number>, Depth extends unknown[] = unknown[], SelectionSet = true, Output = Record<string, unknown>, FirstRecursion extends boolean = false> = (`,
      `${i(1)}N extends Depth['length'] ? // This only occurs when we add a number to the array and this only happends on base case`,
      `${i(1)}(`,
      `${i(2)}NestedType<Output> extends Record<PropertyKey, unknown> ?`,
      `${i(2)}{ [Selection in keyof Common<SelectionSet, NestedType<Output>> as SelectionSet[Selection] extends false | string ? never : NestedType<NestedType<Output>[Selection]> extends Record<PropertyKey, unknown> ? IsEmptyObject<NestedType<NestedType<Output>[Selection]>> extends true ? never : Selection : Selection]: NestedType<Output>[Selection] } : `,
      `${i(2)}Exclude<NestedType<Output>, Record<PropertyKey, unknown>>`,
      `${i(1)}) :`,
      ...
      `${i(4)}(`,
      `${i(5)}OutputFromUnion<SelectionSet[Selection], Output[Selection]> extends Record<PropertyKey, unknown> ? `,
      `${i(5)}BoundedOperationResponse_<RecursionOverride<OutputFromUnion<SelectionSet[Selection], Output[Selection]>, N>, [RecursionOverride<OutputFromUnion<SelectionSet[Selection], Output[Selection]>, N>, ...Depth], SelectionSet[Selection], OutputFromUnion<SelectionSet[Selection], Output[Selection]>> :`,
      `${i(6)}(`,
      `${i(7)}IsUnion<OutputFromUnion<SelectionSet[Selection], Output[Selection]>> extends true ? // Here means we have a union with an object and a primitive`,
      `${i(7)}( // Here we determine if what to do based on selection set and output`,
      `${i(8)}BoundedOperationResponse_<RecursionOverride<OutputFromUnion<SelectionSet[Selection], Output[Selection]>, N>, [RecursionOverride<OutputFromUnion<SelectionSet[Selection], Output[Selection]>, N>, ...Depth], SelectionSet[Selection], OutputFromUnion<SelectionSet[Selection], Output[Selection]>>`,
      `${i(7)}) :`,
      `${i(7)}OutputFromUnion<SelectionSet[Selection], Output[Selection]> // Here we know we just a primitive`,
      `${i(6)})`,
      `${i(6)}// OutputFromUnion<SelectionSet[Selection], NestedType<Output[Selection]>> `,
      `${i(5)}) :`,
      `${i(5)}NestedType<Output[Selection]> extends Record<PropertyKey, unknown> ? `,
      ...
      `${i(1)}) :`,
      `${i(1)}(`,
      `${i(2)}Output extends Record<PropertyKey, unknown> ?`,
      `${i(2)}(`,
      `${i(3)}SelectionSet extends true ? // If true, we don't use Common<SelectionSet, Output> since that will result in {} since SelectionSet is not an object, we don't do this check in First REcursion since we already handle that case in main bounded response type`,
      `${i(3)}{ [Selection in keyof Output as NestedType<Output[Selection]> extends Record<PropertyKey, unknown> ? IsEmptyObject<NestedType<Output[Selection]>> extends true ? never : Selection : Selection]: `,
      `${i(4)}(`,
      `${i(5)}IsUnion<NestedType<Output[Selection]>> extends true ? `,
      `${i(6)}(`,
      `${i(7)}OutputFromUnion<Selection, Output[Selection]> extends Record<PropertyKey, unknown> ? `,
      `${i(7)}BoundedOperationResponse_<N, [N, ...Depth], Selection, OutputFromUnion<Selection, Output[Selection]>> :`,
      `${i(7)}(`,
      `${i(8)}IsUnion<OutputFromUnion<Selection, Output[Selection]>> extends true ? // Here means we have a union with an object and a primitive`,
      `${i(8)}( // Here we determine if what to do based on selection set and output`,
      `${i(9)}BoundedOperationResponse_<N, [N, ...Depth], Selection, OutputFromUnion<Selection, Output[Selection]>>`,
      `${i(8)}) :`,
      `${i(8)}OutputFromUnion<Selection, Output[Selection]> // Here we know we just a primitive`,
      `${i(7)})`,
      `${i(6)}) :`,
      `${i(6)}NestedType<Output[Selection]> extends Record<PropertyKey, unknown> ? `,
      `${i(6)}BoundedOperationResponse_<N, [N, ...Depth], Selection, NestedType<Output[Selection]>> :`,
      `${i(6)}NestedType<Output[Selection]>`,
      `${i(5)})`,
      `${i(4)}} :`,
      `${i(4)}SelectionSet extends Record<PropertyKey, unknown> ?`,
      `${i(4)}{ [Selection in keyof Common<SelectionSet, Output> as SelectionSet[Selection] extends false | string ? never : NestedType<Output[Selection]> extends Record<PropertyKey, unknown> ? IsEmptyObject<NestedType<Output[Selection]>> extends true ? never : Selection : Selection]: `,
      `${i(5)}(`,
      `${i(6)}IsUnion<NestedType<Output[Selection]>> extends true ? `,
      `${i(6)}(`,
      `${i(7)}OutputFromUnion<SelectionSet[Selection], Output[Selection]> extends Record<PropertyKey, unknown> ? `,
      `${i(7)}BoundedOperationResponse_<N, [N, ...Depth], SelectionSet[Selection], OutputFromUnion<SelectionSet[Selection], Output[Selection]>> :`,
      `${i(7)}(`,
      `${i(8)}IsUnion<OutputFromUnion<SelectionSet[Selection], Output[Selection]>> extends true ? // Here means we have a union with an object and a primitive`,
      `${i(8)}( // Here we determine if what to do based on selection set and output`,
      `${i(9)}BoundedOperationResponse_<N, [N, ...Depth], SelectionSet[Selection], OutputFromUnion<SelectionSet[Selection], Output[Selection]>>`,
      `${i(8)}) :`,
      `${i(8)}OutputFromUnion<SelectionSet[Selection], Output[Selection]> // Here we know we just a primitive`,
      `${i(7)})`,
      `${i(6)}) :`,
      `${i(6)}NestedType<Output[Selection]> extends Record<PropertyKey, unknown> ? `,
      `${i(6)}BoundedOperationResponse_<N, [N, ...Depth], SelectionSet[Selection], NestedType<Output[Selection]>> :`,
      `${i(6)}NestedType<Output[Selection]>`,
      `${i(5)})`,
      `${i(4)}} :`,
      `${i(3)}null // If SelectionSet is not true or object`,
      `${i(2)}) :`,
      `${i(2)}SelectionSet extends true ? `,
      `${i(2)}Output : // If Output is not an object`,
      `${i(2)}null `,
      `${i(1)})`,
      `)`,
    ].join("\n"),
    ...
    [
      ...
      `${i(1)}<Operation extends GraphQLOperation, SelectionSet extends BoundedOperationSelectionSetType<GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT>>(operation: Operation, input: Operation extends keyof GraphQLOperationInputWithArguments ? GraphQLOperationInput[Operation] : (Record<string, unknown> | null), selection_set?: SelectionSet | boolean | null | undefined, options?: OperationFetchOptions): Promise<BoundedOperationResponse<SelectionSet, GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT> & OperationFetchError>`,
      `}`,
    ].join("\n"),
    [
    ...
    [
      ...
      `${i(1)}<Operation extends GraphQLOperation, SelectionSet extends BoundedOperationSelectionSetType<GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT>>(operation: Operation, input: Operation extends keyof GraphQLOperationInputWithArguments ? GraphQLOperationInput[Operation] : (Record<string, unknown> | null), selection_set?: SelectionSet | boolean | null | undefined, options?: BoundedLoopFetchOptions): Promise<BoundedOperationResponse<SelectionSet, GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT> & OperationFetchError>`,
      `}`,
    ].join("\n"),
    [
  ...
  if (config.client.gen_hooks) {
    client_types.push(
      [
        `export type UseOperationOverload = {`,
        // `${i(1)}<Operation extends keyof GraphQLOperationInputWithArguments, SelectionSet extends RawSelectionSet = OperationSelectionSet<Operation>>(operation: Operation, input: GraphQLOperationInputWithArguments[Operation], selection_set?: SelectionSet, options?: BoundedLoopFetchOptions): Promise<OperationFetchResponse<Operation, SelectionSet> extends null ? OperationFetchError : OperationFetchResponse<Operation, SelectionSet> & OperationFetchError>`,
        // `${i(1)}<Operation extends keyof GraphQLOperationInputWithoutArguments, SelectionSet extends RawSelectionSet = OperationSelectionSet<Operation>>(operation: Operation, selection_set?: SelectionSet, options?: BoundedLoopFetchOptions): Promise<OperationFetchResponse<Operation, SelectionSet> extends null ? OperationFetchError : OperationFetchResponse<Operation, SelectionSet> & OperationFetchError>`,
        `${i(1)}<Operation extends GraphQLOperation, SelectionSet extends BoundedOperationSelectionSetType<GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT>>(operation: Operation, input: Operation extends keyof GraphQLOperationInputWithArguments ? GraphQLOperationInput[Operation] : (Record<string, unknown> | null), selection_set?: SelectionSet | boolean | null | undefined, options?: OperationFetchOptions & UseMutationOptions<BoundedOperationResponse<SelectionSet, GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT> & OperationFetchError, OperationFetchError, Operation extends keyof GraphQLOperationInputWithArguments ? GraphQLOperationInput[Operation] : (Record<string, unknown> | null)>): UseMutationResult<BoundedOperationResponse<SelectionSet, GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT> & OperationFetchError, OperationFetchError, Operation extends keyof GraphQLOperationInputWithArguments ? GraphQLOperationInput[Operation] : (Record<string, unknown> | null)>`,
        `}`,
      ].join("\n")
    )
    client_types.push(
      [
        `export type UseOperationSWROverload = {`,
        // `${i(1)}<Operation extends keyof GraphQLOperationInputWithArguments, SelectionSet extends RawSelectionSet = OperationSelectionSet<Operation>>(operation: Operation, input: GraphQLOperationInputWithArguments[Operation], selection_set?: SelectionSet, options?: BoundedLoopFetchOptions): Promise<OperationFetchResponse<Operation, SelectionSet> extends null ? OperationFetchError : OperationFetchResponse<Operation, SelectionSet> & OperationFetchError>`,
        // `${i(1)}<Operation extends keyof GraphQLOperationInputWithoutArguments, SelectionSet extends RawSelectionSet = OperationSelectionSet<Operation>>(operation: Operation, selection_set?: SelectionSet, options?: BoundedLoopFetchOptions): Promise<OperationFetchResponse<Operation, SelectionSet> extends null ? OperationFetchError : OperationFetchResponse<Operation, SelectionSet> & OperationFetchError>`,
        `${i(1)}<Operation extends GraphQLOperation, SelectionSet extends BoundedOperationSelectionSetType<GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT>>(operation: Operation, input: Operation extends keyof GraphQLOperationInputWithArguments ? GraphQLOperationInput[Operation] : (Record<string, unknown> | null), selection_set?: SelectionSet | boolean | null | undefined, options?: OperationFetchOptions & SWRConfiguration<BoundedOperationResponse<SelectionSet, GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT> & OperationFetchError, OperationFetchError>): SWRResponse<BoundedOperationResponse<SelectionSet, GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT> & OperationFetchError, OperationFetchError>`,
        `}`,
      ].join("\n")
    )
  }
...
export const gen_clients_functions = async (data: GeneratorData, config: GeneratorConfig): Promise<string[]> => {
  ...
  functions.push([
    ...
    `${i(6)}are_valid.push(val !== null ? val : false)`,
    `${i(5)}}`,
    `${i(5)}else if (typeof selection_set[selection] === "boolean") {`,
    `${i(6)}leaf_selections.push(selection)`,
    `${i(6)}types.push(await recurse_operation_types(selection, GRAPHQL_OPERATION_DATA[operation]["output_types"], [...parent_queue, selection]))`,
    `${i(5)}}`,
    `${i(4)}}`,
    `${i(3)}}))`,
    `${i(2)}}`,
  ...
  functions.push([
    ...
    `${i(2)}const union_select = selection_set["union_select"] ? selection_set["union_select"] : null`,
  ...
  functions.push([
    `/**`,
    ` * Utility function for the [lfd-graphql-client](https://github.com/Luis-Domenech/lfd-graphql-client) package to check if a response has a desired output. `,
    ` * If it does, then it returns it, else it returns undefined.`,
    ` * @param res Output from \`output_fetch\`, \`loop_fetch\` or \`custom_fetch\``,
    ` * @param key Name of key want from the \`output\` or if \`output\` is not an object, then this would be omitted `,
    ` * @returns Returns undefined if the key you want from the output does not exists or if it does, then it returns extactly that`,
    ` */`,
    `export const getOpResult: GetOpResultOverride = <Op extends OperationFetchBaseReturn, Output extends Op['output'], Result extends (Output extends Record<string, any> ? keyof Op['output'] : void)>(res: Op, key?: Result): (Result extends keyof Op['output'] ? Op['output'][Result] : Op['output']) | undefined => {`,
    `${i(1)}if (!res) {`,
    `${i(2)}console.log('Operation Failed!')`,
    `${i(1)}}`,
    `${i(1)}else if (res.fetch_errors) {`,
    `${i(2)}for (const err of res.fetch_errors) {`,
    `${i(3)}console.log(err)`,
    `${i(2)}}`,
    `${i(1)}}`,
    `${i(1)}else {`,
    `${i(2)}if (key) {`,
    `${i(3)}if (!res.output[key]) {`,
    `${i(4)}console.log('Operation Failed!')`,
    `${i(3)}}`,
    `${i(3)}else return res.output[key]`,
    `${i(2)}}`,
    `${i(2)}else {`,
    `${i(3)}if (typeof res.output === 'undefined') {`,
    `${i(4)}console.log('Operation Failed!')`,
    `${i(3)}}`,
    `${i(3)}else return res.output`,
    `${i(2)}}`,
    `${i(1)}}`,
    `${i(1)}return undefined`,
    `}`,
  ].filter(Boolean).join("\n"))
  data.file_data.set('getOpResult', {
    file_name: file_name,
    file_dir: file_dir,
    file_path: file_path,
    same_dir_import_path: same_dir_import_path,
  ...
  add_relative_import('getOpResult', 'GetOpResultOverride', data, config)
  add_relative_import('getOpResult', 'OperationFetchBaseReturn', data, config)
  ...
  functions.push([
    ...
    `${i(1)}const use_conventions = opts.use_conventions !== undefined ? opts.use_conventions : false`,
    ...
    `${i(1)}const headers: Record<string, string> = opts.Authorization ? { 'Content-Type': 'application/json', 'Authoritzation': opts.Authorization } : { 'Content-Type': 'application/json' }`,
    `${i(1)}const log = opts.log ? opts.log : true`,
    `${i(1)}const fetch_options: any = {}`,
    `${i(1)}if (opts.signal) fetch_options['signal'] = opts.signal`,
    `${i(1)}`,
  ...
  functions.push([
    ...
    `${i(1)}const log = opts.log ? opts.log : true`,
    `${i(1)}const fetch_options: any = {}`,
    `${i(1)}const customOperationName = capitalize_first_word(operation) + capitalize_first_word(operation_type)`,
    `${i(1)}const operationName = ${config.client.type_graphql_v2 ? 'customOperationName' : 'operation'}`,
    `${i(1)}if (opts.signal) fetch_options['signal'] = opts.signal`,
    `${i(1)}if (log) console.log(\`Executing operation: \${operation}\`)`,
    `${i(1)}try {`,
    ...
    `${i(3)}if (log) console.log(\`Fetch failed on operation: \${operation}\`)`,
    `${i(3)}return { fetch_errors: ['Selection set is invalid'] } as any`,
    `${i(2)}}`,
    `${i(2)}let operation_request: SimpleGraphQLOperationRequest = { operationName: operationName, query: '' }`,
    ...
    `${i(5)}const operation_input_name = capitalize_first_word(operation) + 'Input'`,
    `${i(5)}operation_request_query.push(\`\${operation_type} \${customOperationName}($data: \${operation_input_name}!) {\`)`,
    ...
    `${i(7)}if (log) console.log(\`Fetch failed on operation: \${operation}\`)`,
    ...
    `${i(5)}})`,
    `${i(5)}operation_request_query.push(\`\${operation_type} \${customOperationName}(\${input_declarations.join(', ')}) {\`)`,
    `${i(5)}operation_request_query.push(\`\${indent(1)}\${operation}(\${input_values.join(', ')}) {\`)`,
    `${i(4)}}`,
    `${i(3)}}`,
    `${i(3)}else {`,
    `${i(4)}operation_request_query.push(\`\${operation_type} \${customOperationName} {\`)`,
    `${i(4)}operation_request_query.push(\`\${indent(1)}\${operation} {\`)`,
    `${i(3)}}`,
    `${i(2)}}`,
    `${i(2)}else {`,
    `${i(3)}operation_request_query.push(\`\${operation_type} \${customOperationName} {\`)`,
    ...
    `${i(2)}const raw_response = await fetch(prod ? endpoint : dev_endpoint, {...fetch_options, method: 'POST', headers: headers, body: JSON.stringify({...operation_request, operationname: \`\${customOperationName}\`})})`,
    `${i(2)}try {`,
    `${i(3)}const json = await raw_response?.json() as any`,
    `${i(3)}if (!json) {`,
    `${i(4)}if (log) console.log(\`Fetch failed on operation: \${operation}\`)`,
    `${i(4)}return { fetch_errors: ['Error calling .json() on fetch response'] } as any`,
    `${i(3)}}`,
    `${i(3)}if (json.errors) {`,
    `${i(4)}if (log) console.log(\`Fetch failed on operation: \${operation}\`)`,
    ...
    `${i(4)}if (!find && operation_response.errors) {`,
    `${i(5)}if (log) console.log(\`Fetch failed on operation: \${operation}\`)`,
    `${i(5)}return { fetch_errors: operation_response.errors.map((e: any) => e.message) } as any`,
    `${i(4)}}`,
    `${i(3)}}`,
    `${i(3)}if (log) console.log(\`Fetch succeeded on operation: \${operation}\`)`,
    // config.client.fixTypeInstantiation ? `${i(3)}// Used to avoid excesively the 'Type instantiation is excessively deep and possibly infinite.' error ` : '',
    // config.client.fixTypeInstantiation ? `${i(3)}// ts-ignore ` : '',
    config.client.hide_type_instantiation_error ? `${i(3)}return { output: operation_response as any }` : `${i(3)}return { output: operation_response }`,
    `${i(2)}}`,
    ...
    `${i(1)}}`,
    `${i(1)}catch(e) {`,
    `${i(2)}if (log) console.log(\`Fetch failed on operation: \${operation}\`)`,
    `${i(2)}return { fetch_errors: [e] } as any`,
    `${i(1)}}`,
    `}`,
  ].filter(Boolean).join("\n"))
  ...
  functions.push([
    ...
    `${i(1)}// eslint-disable-next-line no-constant-condition`,
    `${i(1)}while (true) {`,
    `${i(2)}try {`,
...
export const gen_client_hooks = async (data: GeneratorData, config: GeneratorConfig): Promise<string[]> => {
  if (!config.client.gen_hooks) return []
  ...
  hooks.push([
    `export const useOperation: UseOperationOverload = <Operation extends GraphQLOperation, SelectionSet extends BoundedOperationSelectionSetType<GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT>>(operation: Operation, input: any, selection_set?: SelectionSet | boolean | null | undefined, options?: any) => {`,
    `${i(1)}const opts = options ? options : {}`,
    `${i(1)}return useMutation<BoundedOperationResponse<SelectionSet, GraphQLOperationOutput[Operation], typeof OBJECT_RECURSION_LIMIT> & OperationFetchError, OperationFetchError, Operation extends keyof GraphQLOperationInputWithArguments ? GraphQLOperationInput[Operation] : (Record<string, unknown> | null)>([operation], async () => operation_fetch(operation, input, selection_set, opts), {...opts})`,
    `}`
  ].join("\n"),
  [
    ...
    `${i(1)}return { data, loading, error, mutate, isValidating }`,
    `}`,
  ].join("\n"))
  ...
  const useOperation_file_data = data.file_data.get('useOperation')
  add_import('useMutation', '@tanstack/react-query', false, useOperation_file_data!.imports)
  add_relative_import('useOperation', 'OBJECT_RECURSION_LIMIT', data, config)
  add_relative_import('useOperation', 'operation_fetch', data, config)
  add_relative_import('useOperation', 'BoundedOperationResponse', data, config)
  add_relative_import('useOperation', 'BoundedOperationSelectionSetType', data, config)
  add_relative_import('useOperation', 'GraphQLOperation', data, config)
  add_relative_import('useOperation', 'GraphQLOperationInput', data, config)
  add_relative_import('useOperation', 'GraphQLOperationInputWithArguments', data, config)
  add_relative_import('useOperation', 'GraphQLOperationOutput', data, config)
  add_relative_import('useOperation', 'OperationFetchError', data, config)
  add_relative_import('useOperation', 'OperationFetchOptions', data, config)
  add_relative_import('useOperation', 'UseOperationOverload', data, config)

  const useOperationSWR_file_data = data.file_data.get('useOperationSWR')
  add_import('useSWR', 'swr', true, useOperationSWR_file_data!.imports)
  add_import('SWRConfiguration', 'swr', false, useOperationSWR_file_data!.imports)
  add_relative_import('useOperationSWR', 'UseOperationSWROverload', data, config)
